...
export const AuthProvider = ({ children }) => {
  ...
  const getClassv = async () => {
    const res2 = await axios.get(BaseUrl + `/classInfo/all`,
      {
        headers: { Authorization: `Bearer ${authTokens}` },
      });
    return res2.data
  ...
  const signupUser = async (propic, lid) => {
    if (clsroom) {
    ...
    await axios
      ...
      .then(() => {
        ...
        if (lid === '1') {
          ...
          joy = '/class-submission'
        } else if (lid === '3') {
          joy = '/course-submission'
        } else if (lid.includes('C')) {
          const jorom = lid.split('-')
          console.log(jorom)
          joy = `/classV2/${jorom[1]}`
        } else if (lid.includes('c')) {
          const jorom = lid.split('-')
          console.log(jorom)
          joy = `/courseV2/${jorom[1]}`
        } else if (lid === '4'){
          joy = `/qna`
        }
        standingData(values.email, values.password,joy);
        setLoading(false);
        // }
      })
      .catch((err) => {
  ...
  const loginProcess = async (res, nav = "/profile") => {
    try {
      ...
      localStorage.setItem("phone", JSON.stringify(details.phone_num))
      localStorage.setItem("mail" , JSON.stringify(details.email))
  ...
  const standingData = async (email, password, nav = "/qna") => {
    setLoading(true);
    await axios
      .post(BaseUrl + "/login", {
        email: email,
  ...
  const loginUser = async (e, lid) => {
    ...
    standingData(email, password, joy);
  };
