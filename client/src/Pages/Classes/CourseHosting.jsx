...
import FormWithSections from './CourseInfo';
import ClassPricing from './CoursePricing';
import Verification from './CourseVerification';
import PreviewClass from './CoursePreview';
...
function CourseHosting() {
 ...
 async function nava() {
    ...
    const res = await axios.post(BaseUrl + `/courseInfo`,
    { ...data },
    {
  ...
  const [section1v, setSection1v] = useState({
    ...
    const jett23 = (tab, obj, volva) =>  (event) => {
        ...
        try{
            ...
            if (old){
            console.log("159")
                    old[volva] = typeof event !== 'string' && tab !== 3 ? event.target.value : event 
                    console.log("161")
                    setSection1v(prevObject => ({
                        ...prevObject,
                        [obj]: old
                ...
                console.log('168')
                const value = typeof event !== 'string' && tab !== 3 ? event.target.value : event 
                const jaddddd = []
                jaddddd[volva] = value
                console.log(172)
                setSection1v(prevObject => ({
    ...
    const checkCokk = () => {
      ...
      const savedState2 = Cookies.get('state2cls');
      const savedState3 = Cookies.get('state3cls');
    //   console.log(savedState1, 'SavedCookie')
    }
    
      const saveToCookies = () => {
        // console.log(section1v , 'SavingInCookie')
        Cookies.set('state1cls', JSON.stringify(section1v), { expires: 0.0104 }); // 15 minutes = 0.0104 days
        Cookies.set('state2cls', JSON.stringify(section2v), { expires: 0.0104 });
        Cookies.set('state3cls', JSON.stringify(section3v), { expires: 0.0104 });
        checkCokk()
      };
    ...
    useEffect(() => {
      ...
      if (savedState1) {
        setSection1v(JSON.parse(savedState1));
    }
  ...
  return (
    ...
    <div className="tabs-container mt-4">
      ...
      >
        <StyledTab label="Course Information" className={activeTab === 0 ? 'klout' : ''} />
      ...
      <FormWithSections   infoTaker={jett} infoGetter={gett}  inf2={jett2} inf3={jett23} get2={gett23}/>
