...
const HostSession = () => {
  ...
  function checkIfEventIsAboutToEnd(eventDate) {
    ...
    const [classStart, setSc] = useState(true)
  ...
  useEffect(() => {
    async function getter() {
        ...
        console.log(data)
        const res = await axios.get(BaseUrl + `/classroomv2/sessionInfo/host?id=${roomid}`,
            {
                params: data
            });
  ...
  const [authTokens, setAuthTokens] = useState(() =>
    ...
    React.useEffect(() => {
        async function checkClassroom() {
            const url2 = `${BaseUrl}/register/classroom/host?id=${clid}`
...
<div className="secondDivAtJoinClass">
  <img src={imguu} alt="" />
   ...
   { classStart && <div className={`myCallContainer gl ${opc ? "myCallContainer2" : ""}`}
      ref={myMeeting}
