...
class Courses_Classes {
  ...
  async allCourses() {
    try {
      ...
      const another = await prisma.course_V.findMany({where: {}, 
        include: {
        user: true,
      },orderBy: {
        id: "desc",
    ...
    function sortClassesByEarliestDate(classes) {
      return classes.sort((a, b) => {
          ...
          return bEarliestDate - aEarliestDate; // Swap the order for descending sort
      });
  }

  const savedData = result.concat(another).sort((a, b) => {
    let aEarliestDate = new Date(Math.min(...a.date.map(d => new Date(`${d.date}T${d.time}`))));
    let bEarliestDate = new Date(Math.min(...b.date.map(d => new Date(`${d.date}T${d.time}`))));
    return bEarliestDate - aEarliestDate; // Swap the order for descending sort
})
